...
/**
 * 
 * 说明： HTTP 请求类, 其中代码修改了一点, 和官方的有一点点区别
 * 
...
//		}
	...
	//		System.out.println(i+"------"+"name:"+params[i].getName()+" "+"value:"+params[i].getValue());
	//	}
		for (int i = 0; i < params.length; i++) {
	...
	private static String encodeParameters(PostParameter[] postParams) {
		...
		for (int j = 0; j < postParams.length; j++) {
			if (j != 0)
				buf.append("&");
			try {
				buf.append(URLEncoder.encode(postParams[j].getName(), "UTF-8")).append("=").append(
						URLEncoder.encode(postParams[j].getValue(), "UTF-8"));
